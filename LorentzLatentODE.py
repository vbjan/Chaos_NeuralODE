...
- Potential issue: .permute not view after odeint
"""
...
import time
import logging
from utils import CreationRNN
from utils import get_lr, save_models, load_models

logging.basicConfig(filename='seq2seq_lorenz_prediction/lorenzlatentode.log', level=logging.INFO, )
...
test_data_dir = project_dir + "/data/test/data.h5"
train_data_dir = project_dir + "/data/train/data.h5"
val_data_dir = project_dir + "/data/val/data.h5"
...
def make_prediction_two_rnn(input_len, output_len, latent_dim, traj_stack, encoder_model, latent_model, decoder_model):
    ...
    hid = encoder_model.init_hidden().to(device)
    for i in reversed(
...
def rnn_vae_ode_loss(input_len, out_times, latent_dim, traj_stack, encoder_model, latent_model, decoder_model):
    # Encoding the trajectory
    n_samples = traj_stack.size(0)
    traj_stack = traj_stack.view(n_samples, len(out_times), 1)
    obs_dim = 1
    ...
    traj = decoder_model.forward(latent_traj).view(N_SAMPLES, len(out_times), OBS_DIM)
    if traj.size() != traj_stack.size():
        print(traj.size(), traj_stack.size())
        assert traj.size() == traj_stack.size()
    noise_std_ = torch.zeros(traj.size()).to(device) + .3
    noise_logvar = 2. * torch.log(noise_std_).to(device)
    logpx = log_normal_pdf(traj_stack, traj, noise_logvar).sum(-1).sum(-1)
    ...
    return traj.view(N_SAMPLES, len(out_times), OBS_DIM), loss
...
if __name__ == "__main__":
    ...
    N_SAMPLES = 10  # number of samples f
    ...
    if TRAIN_MODEL:
        if LOAD_THEN_TRAIN:
            load_models(SAVE_DIR, optimizer, EncoderRNN, LatentModel, DecNet)
        ...
        for EPOCH in tqdm(range(EPOCHS)):
            ...
            optimizer.zero_grad()
            pred_traj, loss = rnn_vae_ode_loss(SAMPLE_LEN, LONG_SAMPLE_T, LATENT_DIM, LONG_SAMPLE_TRAJ_STACK,
            ...
            val_traj, val_loss = rnn_vae_ode_loss(SAMPLE_LEN, LONG_SAMPLE_T, LATENT_DIM, VAL_TRAJ_STACK, EncoderRNN,
                                               LatentModel, DecNet)
            ...
            if EPOCH % int(4 * PATIENCE) == 0 and EPOCH != 0:
                save_models(SAVE_DIR, optimizer, EncoderRNN, LatentModel, DecNet)
    ...
    with torch.no_grad():
        ...
        pred_traj, _ = rnn_vae_ode_loss(TEST_LEN, LONG_TEST_T, LATENT_DIM, LONG_TEST_TRAJ_STACK, EncoderRNN,
                                            LatentModel, DecNet)
