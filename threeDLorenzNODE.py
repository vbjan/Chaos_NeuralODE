...
if __name__ == "__main__":
    ...
    if TRAIN_MODEL:
        ...
        train_dataset = DDDLorenzData(train_data_dir, lookahead, tau=1, k=1, n_groups=n_groups)
        ...
        logging.info("                        | augmented dimension: {} | iterations: {} | #trainable parameters: {} \n"
                                             .format(augmentation_dim, n_iterations, get_num_trainable_params(params)))
        ...
        for EPOCH in range(EPOCHS):
            ...
            for n, (batch_X0, batch_X) in enumerate(train_dataloader):
                ...
                loss = torch.mean(torch.abs(X_pred - batch_X)**2) + well_func(augm_X_pred[:, 1:, 3:])
                #print(augm_X_pred[:, 1:, 3:])
                #print(well_func(augm_X_pred[:, :, 3:]))
                loss.backward()
                optimizer.step()
            ...
            if EPOCH % 2 == 0:
                ...
                logging.info("EPOCH {} finished with training loss: {} | validation loss: {} | lr: {} | delta time: {}s"
                      .format(EPOCH, loss, val_loss, get_lr(optimizer), time_for_epochs))
    ...
    with torch.no_grad():
        ...
        print("\nAugmented part of predicted traj: ", augm_ex_traj[:, 3:], "\n")
        ...
        plt.figure()
        plt.plot(data_time, x, '-o', label='Real x')
        ...
        plt.figure()
        plt.plot(data_time, y, '-o', label='Real y')
        ...
        plt.figure()
        plt.plot(data_time, z, '-o', label='Real z')
